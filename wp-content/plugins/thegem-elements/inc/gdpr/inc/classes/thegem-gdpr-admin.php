...
class TheGemGdprAdmin {
	...
	public function add_menu() {
		add_submenu_page(static::SUBMENU_PAGE_PARENT_SLUG, TheGemGdpr::PLUGIN_NAME, __('Privacy & GDPR'), 'manage_options', static::PAGE_URL, array($this, 'generate_page'));
	}
	...
	public static function get_consents_description_default_value() {
		return array(
			TheGemGdpr::CONSENT_NAME_PRIVACY_POLICY=>__('You have read and agreed to our privacy policy'),
			TheGemGdpr::CONSENT_NAME_YOUTUBE=>__('This website uses YouTube service to provide video content streaming.'),
			TheGemGdpr::CONSENT_NAME_VIMEO=>__('This website includes videos, provided by Vimeo - a video content streaming service.'),
			TheGemGdpr::CONSENT_NAME_GOOGLE_FONTS=>__('This site uses so-called web fonts provided by Google for the uniform font representation.'),
			TheGemGdpr::CONSENT_NAME_GOOGLE_MAPS=>__('This website uses Google Maps service, allowing to display interactive maps.'),
			TheGemGdpr::CONSENT_NAME_TRACKING=>__('This website uses functions of the web analytics service Google Analytics.')
		);
	}
			...
			case TheGemGdprCF7::ID:
				...
				if (!empty($form_ids)) {
					...
					foreach ($form_ids as $form_id) {
						...
						$output .= '<div class="thegem-gdpr-cf7-item-title">'.sprintf(__('Form: %s'), get_the_title($form_id)).'</div>';

						$field = array(TheGemGdprCF7::ID, 'forms', $form_id, 'state');						
						$output .= '<div class="thegem-gdpr-field-box">';
						$output .= '<label for="'.esc_attr($this->get_field_id($field)).'">'.__('Activate for this form:').'</label>';
						...
						$default_value = __('By using this form you agree with the storage and handling of your data by this website.');
						$output .= '<div class="thegem-gdpr-field-box">';
						$output .= '<label for="'.esc_attr($this->get_field_id($field)).'">'.__('Checkbox text').'</label>';
						...
						$output .= '</div>';
						$output .= '<div class="thegem-gdpr-allowed-tag">'.__('You can use:').' '.TheGemGdpr::get_allowed_html_tags_output().'</div>';

						$field = array(TheGemGdprCF7::ID, 'forms', $form_id, 'error_message');
						$default_value = __('Please accept the privacy checkbox.');
						$output .= '<div class="thegem-gdpr-field-box">';
						$output .= '<label for="'.esc_attr($this->get_field_id($field)).'">'.__('Error message:').'</label>';
	...
	public function build_custom_css($option_page) {
		...
		if (!$wp_filesystem->put_contents($file_url, $css_string, FS_CHMOD_FILE)) {
			if (is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->get_error_code()) {
				add_settings_error($option_page, $wp_filesystem->errors->get_error_code(), __('Something went wrong: custom.css could not be created.') . $wp_filesystem->errors->get_error_message(), 'error');
			} elseif (!$wp_filesystem->connect()) {
				add_settings_error($option_page, $wp_filesystem->errors->get_error_code(), __('File could not be created. Connection error.'), 'error');
			} elseif (!$wp_filesystem->is_writable($file_url)) {
				add_settings_error($option_page, $wp_filesystem->errors->get_error_code(), __('File could not be created. Cannot write custom css to "' . $file_url . '".'), 'error');
			} else {
				add_settings_error($option_page, $wp_filesystem->errors->get_error_code(), __('File could not be created. Problem with access.'), 'error');
			}
	...
	public static function check_create_upload_dir($wp_filesystem, $option_page) {
		...
		if (!$wp_filesystem->is_dir($upload_dir)) {
			if (!$wp_filesystem->mkdir($upload_dir, 0777)) {
				add_settings_error($option_page, $wp_filesystem->errors->get_error_code(), __('Not available to create '.$upload_dir.' directory in uploads directory.'), 'error');
				return false;
			}
		}
	...
	private function get_default_value_for_settings() {
		return array (
			'privacy_policy_page' => '',
			'privacy_policy_link_text' => __('Privacy Policy'),
			'cookies_policy_page' => '',
			'cookies_policy_link_text' => __('Cookies Policy'),
			'modal_title' => __('Privacy Preferences'),
			'excerpt_text' => __('When you visit our website, it may store information through your browser from specific services, usually in form of cookies. Here you can change your privacy preferences. Please note that blocking some types of cookies may impact your experience on our website and the services we offer.'),
			'notice_text' => __('This content is blocked. Please review your [gem_privacy_settings_link]Privacy Preferences[/gem_privacy_settings_link].'),
			'save_preferences_button_text' => __('Save Preferences'),
			'consents' => array (
				'privacy-policy' => array (
					'description' => __('You have read and agreed to our privacy policy')
				)
			),
			'overlay_styles' => array(
